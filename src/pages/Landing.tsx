...
export default function Landing() {
  return (
    <PageShell>
      <section className="relative overflow-hidden px-5 pt-20 pb-24 sm:px-8 sm:pt-28">
        <div
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          ...
          <p
            className="animate-fade-up mx-auto mt-6 max-w-xl text-[15px] leading-relaxed text-ink-600 sm:text-base"
          ...
          >
            <Link
              to="/consent"
              className="w-full rounded-full bg-ink-950 px-7 py-3.5 text-[15px] font-medium text-paper shadow-card transition hover:bg-plum-800 sm:w-auto"
            >
              내 관계 흐름 분석하기
            </Link>
            <a
      ...
      <section className="border-t border-ink-100 bg-plum-900 px-5 py-20 text-paper sm:px-8">
        <div className="mx-auto max-w-4xl">
          <div className="mb-12 text-center">
            <p className="text-xs font-semibold tracking-[0.2em] text-gold-400 uppercase">Process</p>
            <h2 className="mt-3 font-serif-kr text-2xl font-semibold sm:text-3xl">3분이면 첫 요약을 받습니다</h2>
          </div>
          <div className="grid gap-8 sm:grid-cols-3">
            {STEPS.map((s) => (
              <div key={s.n}>
                <span className="font-serif-kr text-3xl font-semibold text-gold-400">{s.n}</span>
                <h3 className="mt-3 text-[17px] font-semibold">{s.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-plum-200">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-5 py-24 text-center sm:px-8">
        <div className="mx-auto max-w-xl">
          ...
          <Link
            to="/consent"
            className="mt-8 inline-block rounded-full bg-ink-950 px-8 py-3.5 text-[15px] font-medium text-paper shadow-card transition hover:bg-plum-800"
          >
            무료 분석 시작
          </Link>
        </div>
