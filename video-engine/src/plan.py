...
GRAPHICS_EXTRA = {
 ...
 "g_search_terms": ("chips", None, ["여자랑 할 대화 주제", "소개팅 질문 리스트"], ()),
 ...
 "g_one_topic":    ("bars", "소재 하나로 갈 수 있는 거리",
                    ...
                    ("짧게 끝남", "오래 이어짐")),
 "g_four_given":   ("chips", "방금 상대가 준 것", ["친구", "성수", "카페", "주말"], ()),
 ...
 "g_four_steps":   ("bars", "대화의 네 단계",
                    [("정보", .25, "cool"), ("이유", .52, "cool"),
                     ("기준", .76, "cool"), ("자기 표현", .98, "cool-hot")],
                    ("얕음", "깊음")),
 ...
 "g_interest_know": ("twobranch", "질문을 많이 하면", "관심은 보인다", "이해는 안 늘 수 있다"),
...
# ── the plan ────────────────────────────────────────────────────────────────
PLAN = [
    # ══ 0-27  hook: the first twenty minutes, then nothing ══════════════════
    ("I", "ico_stopwatch|처음 20분은 쉽습니다", 3),      # 0-2
    ("Q", "m|m|m", 3),                                   # 3-5
    ("B", "br_cafe_two", 1),                             # 6
    ("P", "badq|문제는 그 다음입니다", 1),                     # 7
    ...
    ("T", "", 1),                                        # 20  "또 뭐 있지?"
    ("B", "br_street", 3),                               # 21-23
    ("B", "br_search_list", 2),                          # 24-25
    ("G", "g_search_terms", 2),                          # 26-27

    # ══ 28-51  it is not a shortage of topics ═══════════════════════════════
    ("H", "01|소재의 착각", 2),                           # 28-29
    ("P", "fewer|소재가 부족한 게 아닙니다", 2),                 # 30-31
    ...
    ("P", "think|할 얘기가 굉장히 많습니다", 1),                 # 46
    ("G", "g_run_deeper", 3),                            # 47-49
    ("N", "neon_tree", 1),                               # 50
    ("G", "g_one_topic", 1),                             # 51

    # ══ 52-84  the questions are not connected ══════════════════════════════
    ("P", "notrick|새 주제를 몰라서가 아닙니다", 2),               # 52-53
    ("Y", "br_lean_listen|방금 한 말 안에", 2),           # 54-55
    ("P", "distance|이 차이가 큽니다", 1),                         # 56
    ...
    ("P", "spoil|네 가지를 다 버렸습니다", 2),                   # 78-79
    ("I", "ico_clock|10분 뒤", 1),                        # 80
    ("T", "", 1),                                        # 81  "왜 이렇게 할 말이 없지?"
    ("P", "listen|할 말은 계속 오고 있습니다", 3),                # 82-84

    # ══ 85-116  why it happens: listening while doing four other things ═════
    ("H", "02|듣지 못하는 이유", 1),                       # 85
    ("P", "notrick|화술의 문제가 아닙니다", 2),                    # 86-87
    ...
    ("P", "bored|당연히 버벅입니다", 1),                         # 107
    ("B", "br_counsel", 1),                              # 108
    ("Q", "m", 3),                                       # 109-111
    ("G", "g_20_40", 2),                                 # 112-113
    ("N", "neon_loop", 2),                               # 114-115
    ("P", "commute|대화 방식은 그대로입니다", 1),                  # 116

    # ══ 117-166  go one step down: the Japan example ════════════════════════
    ...
    ("Q", "w", 3),                                       # 132-134
    ("Q", "m", 3),                                       # 135-137
    ("P", "calm|이것도 괜찮습니다", 1),                         # 138
    ("G", "g_depth_flat", 2),                            # 139-140
    ("N", "neon_stretch", 1),                            # 141
    ("Q", "m", 1),                                       # 142
    ("T", "이유", 1),                                     # 143
    ("Q", "w", 3),                                       # 144-146
    ("N", "neon_stretch", 1),                            # 147
    ("Q", "m|m", 2),                                     # 148-149
    ("T", "취향과 기준", 1),                               # 150
    ("Q", "w|w|w", 5),                                   # 151-155
    ("Y", "br_japan_walk|이 사람의 여행 방식", 1),         # 156
    ("P", "pin|여기가 중요합니다", 2),                         # 157-158
    ("Q", "m|m|m|m", 4),                                 # 159-162
    ("C", "woman_b@dark", 1),                            # 163
    ("Q", "w", 2),                                       # 164-165
    ("Q", "m", 1),                                       # 166

    # ══ 167-185  the four levels ════════════════════════════════════════════
    ("B", "br_two_laugh", 1),                            # 167
    ("P", "family|서로 사람이 보였습니다", 2),                    # 168-169
    ("T", "네 단계", 1),                                  # 170
    ("G", "g_four_steps", 4),                            # 171-174
    ("G", "g_depth_flat", 2),                            # 175-176
    ("G", "g_depth_layers", 2),                          # 177-178
    ("I", "ico_links|자기 표현까지", 2),                   # 179-180
    ("P", "pin|이게 중요합니다", 1),                           # 181
    ("G", "g_info_exchange", 4),                         # 182-185

    # ══ 186-230  the pilates example ════════════════════════════════════════
    ("B", "br_pilates", 1),                              # 186
    ("Q", "w", 2),                                       # 187-188
    ("T", "정보 질문만 하면", 1),                          # 189
    ("Q", "m|m|m", 4),                                   # 190-193
    ("P", "calm|물론 필요합니다", 1),                           # 194
    ("N", "neon_bridge", 1),                             # 195
    ("Q", "m|m", 2),                                     # 196-197
    ("Q", "w|w", 4),                                     # 198-201
    ("Q", "m|m", 4),                                     # 202-205
    ...
    ("T", "감정", 1),                                     # 212
    ("Q", "m|m", 3),                                     # 213-215
    ("P", "self|여러분 이야기도 들어갑니다", 1),                 # 216
    ("T", "필라테스 하나로", 1),                           # 217
    ("G", "g_pilates_web", 6),                           # 218-223
    ("P", "hobby|계속 이어집니다", 1),                           # 224
    ("C", "man_anx@dark", 1),                            # 225
    ("Q", "w", 2),                                       # 226-227
    ("Q", "m", 2),                                       # 228-229
    ("I", "ico_trash|또 버립니다", 1),                     # 230

    # ══ 231-265  catch three words ══════════════════════════════════════════
    ...
    ("Q", "m", 2),                                       # 252-253
    ("P", "door|밖에서 가져올 필요가 없습니다", 2),              # 254-255
    ...
    # ══ 266-310  tail questions alone become an interrogation ═══════════════
    ("N", "neon_dial", 1),                               # 266
    ("Q", "m", 3),                                       # 267-269
    ("P", "badq|그것도 아닙니다", 2),                           # 270-271
    ...
    ("P", "listen|바로 다음 질문 대신", 2),                       # 285-286
    ("Q", "w", 3),                                       # 287-289
    ("Q", "m", 2),                                       # 290-291
    ("Q", "m|m", 4),                                     # 292-295
    ("Q", "m|m", 4),                                     # 296-299
    ...
    # ══ 311-345  showing interest is not knowing someone ════════════════════
    ("H", "05|관심과 이해", 2),                            # 311-312
    ("G", "g_interest_know", 2),                          # 313-314
    ("P", "radar|관심 있어 보입니다", 2),                        # 315-316
    ("P", "mirror|깊게 알게 되는 건 아닙니다", 2),                 # 317-318
    ...
    # ══ 346-390  a topic moves, it is not swapped ═══════════════════════════
    ("Q", "m", 2),                                       # 346-347
    ("T", "지금 둘의 상황으로", 2),                        # 348-349
    ("Q", "w", 3),                                       # 350-352
    ("P", "spoil|억지로 바꾼 게 아닙니다", 2),                    # 353-354
    ("Y", "il_topic_map|흘러간 겁니다", 2),                # 355-356
    ("B", "br_counsel", 2),                              # 357-358
    ("T", "바꾸는 게 아니라 이동", 2),                      # 359-360
    ("G", "g_topic_chain", 5),                           # 361-365
    ("P", "commute|이렇게 이동합니다", 1),                          # 366
    ("B", "br_two_laugh", 1),                            # 367
    ("G", "g_drift", 4),                                 # 368-371
    ("P", "relief|뜬금없지 않습니다", 2),                          # 372-373
    ("T", "반대로", 1),                                    # 374
    ...
    # ══ 391-436  the two seconds of silence ═════════════════════════════════
    ("H", "06|2초의 공백", 1),                             # 391
    ...
    ("P", "fewer|질문을 덜 하는 게 아닙니다", 2),                 # 419-420
    ("P", "radar|더 정확한 순간에", 2),                          # 421-422
    ("G", "g_gap_link", 2),                              # 423-424
    ("G", "g_bar_ease", 4),                              # 425-428
    ("B", "br_review_note", 2),                          # 429-430
    ("I", "ico_notebook|질문 목록을 내려놓기", 2),          # 431-432
    ("Q", "m", 3),                                       # 433-435
    ("P", "distance|눈앞의 사람을 놓칩니다", 1),                     # 436

    # ══ 437-473  the drill: fact, feeling, standard ═════════════════════════
    ...
    # ══ 474-518  a topic is not only an event ═══════════════════════════════
    ("P", "calendar|소재를 사건으로만 봅니다", 3),                   # 474-476
    ("G", "g_events", 5),                                # 477-481
    ("T", "사건만 있는 게 아닙니다", 2),                    # 482-483
    ("G", "g_nonevents", 4),                             # 484-487
    ("T", "이 모든 게 소재입니다", 1),                      # 488
    ("P", "differ|다르다는 것도 소재입니다", 2),                   # 489-490
    ("Q", "w", 3),                                       # 491-493
    ("Q", "m", 3),                                       # 494-496
    ("Q", "m|m|m", 4),                                   # 497-500
    ("Q", "w|w", 4),                                     # 501-504
    ...
    ("T", "계속 이동합니다", 1),                            # 515
    ("P", "house|달라도 문제가 없습니다", 2),                     # 516-517
    ("T", "차이가 소재가 됩니다", 1),                       # 518

    # ══ 519-543  erasing the difference ═════════════════════════════════════
    ("C", "man_anx@dark", 2),                            # 519-520
    ("N", "neon_split", 1),                              # 521
    ("G", "g_mirror_yes", 4),                            # 522-525
    ("P", "relief|갈등은 없습니다", 2),                            # 526-527
    ("Q", "m", 3),                                       # 528-530
    ("P", "mirror|같은 상태가 아닙니다", 2),                       # 531-532
    ...
    # ══ 544-575  three kinds of question, then yourself ═════════════════════
    ("I", "ico_layers4|질문은 세 종류", 3),                # 544-546
    ("P", "wonder|첫 번째 · 사실 질문", 1),                # 547
    ("Q", "m|m|m", 3),                                   # 548-550
    ("T", "시작할 때 필요합니다", 1),                       # 551
    ("P", "think|두 번째 · 이유 질문", 1),                 # 552
    ("Q", "m|m|m", 3),                                   # 553-555
    ("T", "사람의 생각이 나옵니다", 1),                     # 556
    ("P", "differ|세 번째 · 기준 질문", 1),                # 557
    ("Q", "m|m|m|m", 4),                                 # 558-561
    ("T", "가치관이 나옵니다", 1),                          # 562
    ("P", "self|마지막은 자기표현", 2),                     # 563-564
    ("Q", "m|m|m", 3),                                   # 565-567
    ("T", "왔다 갔다 하면 됩니다", 1),                      # 568
    ("G", "g_flow_five", 5),                             # 569-573
    ("T", "기본 흐름입니다", 2),                            # 574-575

    # ══ 576-602  depth is a dial, not a destination ═════════════════════════
    ("T", "다 깊게 할 필요는 없습니다", 3),                 # 576-578
    ("Q", "m", 2),                                       # 579-580
    ("P", "bored|부담스럽습니다", 1),                             # 581
    ...
    ("P", "door|짧게 끝나면 놓아주기", 2),                       # 599-600
    ("G", "g_depth_wave", 2),                            # 601-602

    # ══ 603-628  three things to practise, then review ══════════════════════
    ("P", "medal|100개를 외울 필요가 없습니다", 2),               # 603-604
    ("T", "연습할 건 세 가지", 1),                          # 605
    ("G", "g_practice3", 3),                             # 606-608
    ("P", "travel|대화가 꽤 달라집니다", 2),                       # 609-610
    ("I", "ico_notebook|복기", 2),                         # 611-612
    ("G", "g_review4", 4),                               # 613-616
    ("P", "calendar|대화 패턴이 보입니다", 1),                       # 617
    ("Q", "m|m", 3),                                     # 618-620
    ...
    # ══ 629-667  final takeaway and CTA ═════════════════════════════════════
    ("H", "09|오늘의 정리", 1),                            # 629
    ("Q", "m", 3),                                       # 630-632
