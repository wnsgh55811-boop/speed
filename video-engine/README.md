...
### v2 리뷰에서 고친 원칙 (다음 영상에도 그대로)

- **핵심 타이포는 화면에 한 줄만** — `steps_html`: 문장마다 한 줄이 나오고, 다음 줄은
  이전 줄이 완전히 사라진 뒤에 들어온다(겹치는 크로스페이드 금지). 2줄은 `|` 로 명시할 때만.
- **텍스트에 느린 줌 금지** — 씬 전체 scale 트윈(`fgm`)을 없앴다. 30fps 에서 떨려 보인다.
  움직임은 사진 Ken Burns 와 아이콘 float 에만.
- **흰 글씨 뒤는 항상 불투명** — pill·카드·패널은 반투명 흰 배경 금지, 짙은 불투명 배경.
  사진 위 문장은 `dim` 베일(0.5~0.7)로 대비 확보. `.kt.c-am` 처럼 색 클래스 우선순위 주의.
- **라벨은 말할 때만** — 내레이션이 이미 말하는 제목·태그·킥은 붙이지 않는다.
- **저울 의미** — 무거운 쪽(갑, 주도권을 가진 쪽)이 내려간다. 넘겨준 선택권은 그쪽 접시에 쌓인다.
- **누끼** — `scripts/cutout.py`(아이콘은 플러드필, 인물·동물은 rembg) → `cut_*.png`.
  허리에서 잘린 인물 컷은 화면 하단에 붙인다. 어두운 피사체는 감마로 그림자를 들어 올린다
