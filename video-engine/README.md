...
- **presigned PUT URL 을 손으로 옮기면 400** — 긴 URL 전사 오류가 잦았다(5개 중 3번).
  업로드 실패는 치명적이지 않게 두고, 실패분만 새 URL 로 즉시 재업로드.
- **샌드박스는 백그라운드 작업 1개당 ~15분** — 작업당 설정 1.2분 + 구간(약 170초 분량) 2개가 한계.
